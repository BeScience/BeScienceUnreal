...
std::map<int, ABSU_TrackingHand::TrackedRect> ABSU_TrackingHand::getTrackedRects(std::map<int, TrackedRect> trackedRects, std::vector<BSU_Blaze::PalmDetection> filteredDets, uint32_t& nextId)
{
    std::vector<int> foundIndex;

    for (const BSU_Blaze::PalmDetection& filteredDet : filteredDets)
    {
...
void ABSU_TrackingHand::Inference()
{
    ...
    */
    blaze.ResizeAndPad(cvImage, img256, img128, scale, pad);
    //UE_LOG(LogTemp, Log, TEXT("scale value: %f, pad value: (%f, %f)"), scale, pad[0], pad[1]);
    std::vector<BSU_Blaze::PalmDetection> normDets = blaze.PredictPalmDetections(img128);
    std::vector<BSU_Blaze::PalmDetection> denormDets = blaze.DenormalizePalmDetections(normDets, webcamWidth, webcamHeight, pad);
    std::vector<BSU_Blaze::PalmDetection> filteredDets = blaze.FilteringDets(denormDets, webcamWidth, webcamHeight);
