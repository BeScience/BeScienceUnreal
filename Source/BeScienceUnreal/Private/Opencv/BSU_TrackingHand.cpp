...
#include "Opencv/BSU_TrackingHand.h"

// Called when the game starts or when spawned
void ABSU_TrackingHand::BeginPlay()
{
	Super::BeginPlay();
...
float ABSU_TrackingHand::calculateIOU(const cv::Rect& rect1, const cv::Rect& rect2)
{
...
std::map<int, ABSU_TrackingHand::TrackedRect> ABSU_TrackingHand::getTrackedRects(std::map<int, TrackedRect> trackedRects, std::vector<cv::Rect> skinRegions, uint32_t& nextId)
{
...
std::map<int, ABSU_TrackingHand::TrackedRect> ABSU_TrackingHand::getResRects(std::map<int, TrackedRect> trackedRects)
{
...
void ABSU_TrackingHand::drawTrackedRect(cv::Mat frame, std::pair<int, TrackedRect> trackedRect)
{
    cv::putText(
...
cv::Mat ABSU_TrackingHand::overlayTransparent(const cv::Mat& background_img, const cv::Mat& img_to_overlay_t, int x, int y)
{
...
std::vector<cv::Rect> ABSU_TrackingHand::getSkinRegions(cv::Mat frame, cv::Rect roi, cv::Scalar lowerBound,
    cv::Scalar upperBound, cv::Mat kernel)
{
    cv::Mat frameHSV;
...
float ABSU_TrackingHand::sigmoid(float x)
{
    return 1 / (1 + exp(-x));
}

ABSU_TrackingHand::DetectResult ABSU_TrackingHand::getDetectResult(cv::Mat& frame, cv::Mat regressor,
    cv::Mat classificator, int stride, int anchor_count, int column, int row, int anchor, int offset,
...
std::vector<ABSU_TrackingHand::DetectResult> ABSU_TrackingHand::getDetectResults(cv::Mat frame, cv::dnn::Net net,
    cv::Size detectInputSize, cv::Size detectOutputSize)
{
...
void ABSU_TrackingHand::drawDetectResult(cv::Mat frame, DetectResult res, cv::Rect objDetectRoi)
{
    cv::putText(
...
void ABSU_TrackingHand::checkIsHand(std::map<int, TrackedRect>& trackedRects, std::vector<DetectResult>& detectResults, cv::Rect objDetectRoi)
{
    for (auto& trackedRect : trackedRects)
    {
...
cv::Mat ABSU_TrackingHand::drawAndGetRotatedPoints(cv::Mat frame, cv::Point2f wheelStartPoint, cv::Mat points,
    cv::Mat rotationMat)
{
    // 좌표 회전 적용
...
void ABSU_TrackingHand::wheelCheck(cv::Mat frame, bool& wheelLeftChecked, bool& wheelRightChecked, cv::Point2f wheelStartPoint, cv::Mat rotatedPoints, std::map<int, TrackedRect> resRects)
{
    // 회전된 좌표 추출
    cv::Point2f rotatedLeftPoint(rotatedPoints.at<cv::Vec2d>(0, 0)[0], rotatedPoints.at<cv::Vec2d>(0, 0)[1]);
    cv::Point2f rotatedRightPoint(rotatedPoints.at<cv::Vec2d>(1, 0)[0], rotatedPoints.at<cv::Vec2d>(1, 0)[1]);
...
float ABSU_TrackingHand::getWheelAngle(std::map<int, TrackedRect> resRects)
{
...
void ABSU_TrackingHand::setHandPos(std::map<int, TrackedRect> resRects)
{
    TrackedRect leftRect = resRects.begin()->second;
    TrackedRect rightRect = resRects.begin()->second;
...
std::vector<ABSU_TrackingHand::Button> ABSU_TrackingHand::initButtons()
{
...
void ABSU_TrackingHand::drawButtons(cv::Mat frame, std::vector<Button> buttons)
{
...
bool ABSU_TrackingHand::isPointInsideRect(const cv::Point2d& point, const cv::Rect& rect)
{
    return (point.x >= rect.x && point.x < rect.x + rect.width &&
       point.y >= rect.y && point.y < rect.y + rect.height);
}

bool ABSU_TrackingHand::checkBoolVector(const std::vector<bool>& vec)
{
    for (bool value : vec) 
    {
        if (value)
...
void ABSU_TrackingHand::buttonEvent(cv::Mat frame, std::map<int, TrackedRect> resRects, std::vector<Button>& buttons, bool& isOutOfButton, std::chrono::steady_clock::time_point& startTime)
{
...
void ABSU_TrackingHand::ReadFrame()
{
    ...
    Inference();
    imageTexture = MatToTexture2D(cvImage);
}

void ABSU_TrackingHand::Inference()
{
    auto time_start = std::chrono::steady_clock::now();
    cv::flip(cvImage, cvImage, 1);

    /// 손 검출 파트
    std::vector<cv::Rect> skinRegions = getSkinRegions(cvImage, handRoiRect, skinLowerBound, skinUpperBound, morphKernel);
    //skinRegions로 trackedRects 구함(등록, 생명주기 확인, 제거)
    trackedRectsMap = getTrackedRects(trackedRectsMap, skinRegions, nextIndex);
    // 객체 탐지 및 결과 반환
    cv::Mat detectFrame = cvImage(objDetectRoiRect).clone();
    std::vector<DetectResult> detectResults = getDetectResults(detectFrame, net_det, detectInput, detectOutput);

    /// 머리 자세 추론 파트
    cv::Mat head_area = cvImage(head_roi);
    ...
    if (visualize(head_area, faces, image_points_2D))
    {
        ...
        cv::circle(head_area, cv::Point(int(image_points_2D.at<double>(5, 0)),
            int(image_points_2D.at<double>(5, 1))), 2, cv::Scalar(255, 255, 255), 2);
    }

    if (cv::solvePnPRansac(figure_points_3D, image_points_2D, camera_matrix,
        distortion_coeff, vector_rotation, vector_translation))
    {
        std::vector<cv::Point2d> pose_points = get_pose_points(image_points_2D, vector_rotation,
            vector_translation, camera_matrix, distortion_coeff);
        ...
        cv::putText(head_area, cv::format("%d", x_diff), cv::Point(pose_points[0].x - 10, pose_points[0].y - 20),
        ...
        if (xdiff_vector.size() == 15)
        {
            ...
            cv::putText(head_area, cv::format("%.1f", cliped_xdiff), cv::Point(pose_points[0].x + 40, pose_points[0].y - 20),
    ...
    for (auto& detectResult : detectResults)
    {
        drawDetectResult(cvImage, detectResult, objDetectRoiRect);
    }
    ...
    for (auto& trackedRect : resRects)
    {
        drawTrackedRect(cvImage, trackedRect);
    }

    cv::rectangle(cvImage, handRoiRect, cv::Scalar(255, 0, 0), 1);  // 사각형 그리기
    cv::rectangle(cvImage, objDetectRoiRect, cv::Scalar(0, 255, 0), 1);  // 사각형 그리기
    ...
    // 좌표 회전 적용
    cv::Mat rotatedPoints = drawAndGetRotatedPoints(cvImage, wheelStartPos, wheelPoints, rotationMat);
    ...
    if (resRects.size() == 2)
    {
        ...
        setHandPos(resRects);
        wheelCheck(cvImage, wheelLeftChecked, wheelRightChecked, wheelStartPos, rotatedPoints, resRects);
    ...
    buttonEvent(cvImage, resRects, controlButtons, isOutOfControlButton, startTimeControlButton);

    //버튼 그리기
    drawButtons(cvImage, controlButtons);

    cvImage = overlayTransparent(cvImage, rotatedImage, 200, 240);
    ...
    cv::putText(cvImage, time_spent, cv::Point(0, 50), cv::FONT_HERSHEY_SIMPLEX, 2, cv::Scalar(125, 125, 125), 2);
}

UTexture2D* ABSU_TrackingHand::MatToTexture2D(const cv::Mat InMat)
{
...
cv::Mat ABSU_TrackingHand::get_image_points_2D()
{
...
cv::Mat ABSU_TrackingHand::get_figure_points_3D()
{
...
cv::Mat ABSU_TrackingHand::get_camera_matrix()
{
...
cv::Mat ABSU_TrackingHand::get_distortion_coeff()
{
...
void ABSU_TrackingHand::estimate_chin(cv::Mat& image_points_2D)
{
...
bool ABSU_TrackingHand::visualize(cv::Mat& frame, const cv::Mat& faces, cv::Mat& image_points_2D)
{
    bool res = false;
...
std::vector<cv::Point2d> ABSU_TrackingHand::get_pose_points(cv::Mat& image_points_2D, cv::Mat& vector_rot, cv::Mat& vector_tran, cv::Mat& camera_mat, cv::Mat& distort_coeff)
{
...
float ABSU_TrackingHand::clip_avg(std::vector<int> xdiff_vec)
{
