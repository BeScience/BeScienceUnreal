...
#include "BSU_TrackingHand.generated.h"

UCLASS()
class BESCIENCEUNREAL_API ABSU_TrackingHand : public AActor
{
	GENERATED_BODY()
...
public:	
	...
	bool visualize(cv::Mat& frame, const cv::Mat& faces, cv::Mat& image_points_2D);
	std::vector<cv::Point2d> get_pose_points(cv::Mat& image_points_2D, cv::Mat& vector_rot, cv::Mat& vector_tran, cv::Mat& camera_mat, cv::Mat& distort_coeff);
	float clip_avg(std::vector<int> xdiff_vec);
